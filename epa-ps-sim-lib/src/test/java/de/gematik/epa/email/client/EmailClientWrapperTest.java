 ...
 * #L%
 */
package de.gematik.epa.email.client;
...
class EmailClientWrapperTest {
  private final String serverUrl = "http://localhost:8080";
  private final String userAgent = "ps-sim";
  private final JaxRsClientWrapper<EmailManagementApi> emailClientWrapper =
      new JaxRsClientWrapper<>(serverUrl, userAgent, EmailManagementApi.class);

  @Test
  void shouldInitializeEmailManagementApi() {
    final EmailManagementApi api = emailClientWrapper.getServiceApi();
  ...
  void shouldThrowExceptionWhenServerUrlIsNull() {
    final Exception exception =
        assertThrows(
            ...
            () -> new JaxRsClientWrapper<>(null, userAgent, EmailManagementApi.class));
  ...
  void shouldHandleEmptyServerUrl() {
    final JaxRsClientWrapper<EmailManagementApi> wrapper =
        new JaxRsClientWrapper<>("", "", EmailManagementApi.class);
    final EmailManagementApi api = wrapper.getServiceApi();
    assertThat(api).isNotNull();
  }
}
